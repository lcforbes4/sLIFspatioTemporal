...
%%%%%%%%%%%
J1_grid = zeros(maxstep,1); J1_grid(1) = J1; 
vstar = (J0+sqrt(J0^2+4*(E0-J0)))/2;
cgrid = J1_grid; cgrid(1) = c0;
l2grid = J1_grid; l2grid(1) = sqrt(dx)*norm(u(1:end-2)-vstar,2);
SOL_grid = [u;J1];

mingrid = J1_grid; mingrid(1) = min(u(1:end-1));
maxgrid = J1_grid; maxgrid(1) = max(u(1:end-1));
...
for pp = par_direc
    ...
    sec = [zeros(N+1,1);pp];
    zold = [u;J1]; % The initial solution
    ...
    %kernel
    J = (J0 + z0(end)*cos(x))/ 2/pi;
    Jhat = fft(J);

    %%%%%BEGIN continuation %%%%%
    while (steps < maxstep) && (sqrt(dx)*norm(z0(1:end-2)-( (J0+sqrt(J0^2+4*(E0-J0)))/2),2)>0.05)
        ii=ii+1;
        ...
        tic
        fprintf(['starting Newton with J1 = ' num2str(zinit(end))])
        while (nresidual>tol)
            % define jacobian:
            dgt = @(dz) dlif_z_J1(dz,z0,zinit,sec,Jhat,f,f1,k,fftcx,sx,N);
        ...
        disp(['#Newton iters=' num2str(niter-1)  ', J1=' num2str(z0(end)) ', c=' num2str(z0(end-1)) ', time=' num2str(toc)]) % output summary from this step
        %%%%%%%%%%%%%%%%%%%%%END Newton Loop%%%%%%%%%%%%%
        ...
        switch newton_control
            ...
            otherwise %if everything went fine
                steps = steps+1;J11 = z0(end);
                J1_grid(steps) = J11; vstar = (J0+sqrt(J0^2+4*(E0-J0)))/2;
                ...
                subplot(4,1,3)
                plot(J1_grid(1:steps),l2grid(1:steps),'.-')
                xlabel('$J1$','Interpreter','latex')
                ylabel('$\| v - v_*\|_{L^2}$','Interpreter','latex')

                subplot(4,1,4)
                plot(J1_grid(1:steps),mingrid(1:steps),'b.-')
                hold on
                plot(J1_grid(1:steps),maxgrid(1:steps),'r.-')
                xlabel('$J1$','Interpreter','latex')
    ...
    if pp == par_direc(1)
        J1_grid(1:steps) = flipud(J1_grid(1:steps));
...
E = E0;
save('lif_data_J1.mat','J1_grid','cgrid','l2grid','SOL_grid','mingrid','maxgrid','N','x','L','E','J0')
